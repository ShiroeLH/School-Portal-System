...
if (isActionAccessible($guid, $connection2, '/modules/School Admin/rollGroup_manage_edit.php') == false) {
    ...
    if ($gibbonRollGroupID == '' or $gibbonSchoolYearID == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            $row = $form->addRow();
                $row->addLabel('tutors', __('Teachers'))->description(__('Up to 3 per roll group. The first-listed will be marked as "Main Tutor".'));
                ...
                $column->addSelectStaff('gibbonPersonIDTutor5')->placeholder();
                $column->addSelectStaff('gibbonPersonIDTutor6')->placeholder();
                $column->addSelectStaff('gibbonPersonIDTutor7')->placeholder();
                $column->addSelectStaff('gibbonPersonIDTutor8')->placeholder();
